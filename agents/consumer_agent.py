...
class ConsumerAgentDQN:
    ...
    def __init__(
        ...
        self.type = self.building_type  # alias for compatibility
    ...
    def _process_agg_signal(self, agg_signal):
        ...
        Returns numpy array dtype float32, shape (2,)
        """
        ...
        # dict-like with nonce/ciphertext (aggregator.encrypt returns this)
    ...
    # ---------------------------
    # public: decrypt_signal (used by main.py)
    # ---------------------------
    def decrypt_signal(self, agg_packet):
        """Public helper so main.py can pass in whatever the aggregator produced."""
        return self._process_agg_signal(agg_packet)
    ...
    def select_action(self, obs):
        ...
        try:
            low = float(self.action_space.low) if np.isscalar(self.action_space.low) \
                else float(np.atleast_1d(self.action_space.low)[0])
            high = float(self.action_space.high) if np.isscalar(self.action_space.high) \
                else float(np.atleast_1d(self.action_space.high)[0])
    ...
    # reward (kept simple)
    # ---------------------------
    def get_reward(self, raw_reward, current_power_demand, action_value,
                   soc_before_action, electricity_pricing, agg_signal=None):
        ...
        # SoC constraints
        if (action_scalar < 0 and soc_before_action <= 0.0) or \
           (action_scalar > 0 and soc_before_action >= 1.0):
            reward_constraint = -1.0

        # Emergency mode: override reward shaping to prioritize critical load
        if self.emergency_flag:
            try:
        ...
        # Price-based shifting
        reward_shifting = - (electricity_pricing * current_power_demand) * 0.01

        # Follow aggregator-provided price signal if present
        agg_follow = 0.0
        ...
        if hasattr(self.building, 'indoor_dry_bulb_temperature') and len(self.building.indoor_dry_bulb_temperature):
            self.satisfaction_history.append(
                1.0 - abs(self.building.indoor_dry_bulb_temperature[-1] - 22.0) / 10.0
            )
        else:
        ...
        try:
            ...
            if os.path.exists(policy_path):
                ...
                self.target_net.load_state_dict(
                    torch.load(os.path.join(directory, f"consumer_{self.id}_target_ep{episode}.pth"), map_location="cpu")
                )
                try:
                    self.optimizer.load_state_dict(
                        torch.load(os.path.join(directory, f"consumer_{self.id}_optim_ep{episode}.pth"), map_location="cpu")
                    )
                except Exception:
                    pass
    ...
    def reset_episode(self):
        ...
        # NOTE: we do NOT reset epsilon here, so it decays across episodes.
